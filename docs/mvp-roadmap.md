...
| S2.7 — Stable diagnostics | **Partial.** Existing [diagnostic fields](../packages/compiler/src/public-types.ts) remain unchanged. [Public parser-boundary regressions](../packages/compiler/test/parser-boundary.test.ts) pin mixed-list/nested malformed syntax, existing code/id, path-clean syntax messages and LKG; selector-parser author exceptions no longer escape. | Public source ranges/path attribution, deterministic range/code/path sorting and matrix-wide diagnostics/rollback. Diagnostic id intentionally remains caller-supplied (possibly physical); internal spans do not complete the public source-position contract. |
