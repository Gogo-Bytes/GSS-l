...
| S4.9 — Interleaved chains/global branches | **Partial.** [Session tests](../packages/compiler/test/compiler-session.test.ts#L1735) cover one root-level `+` runtime edge followed by two ownership-descendant edges in `.input + .label .icon .badge`; the full target path and intermediate/final markers are retained, declaration reversal is deterministic, and another runtime edge still fails closed. Parent native acceptance passed **86 fixtures / 1011 comparisons**, zero differences/literal failures, and all twelve controls; the earlier **84 / 1007** result remains evidence only for the one-descendant slice. New [deep native fixtures](../packages/testing/browser/interleaved-chain-fixtures.ts) verify the two-descendant slice. The parser still has no explicit-global functional/observed branch. | Other interleaved runtime/prefix shapes and explicit-global functional/observed branches, with positive and ambiguity/rollback fixtures. |
