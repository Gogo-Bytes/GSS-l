...
| --- | --- | --- |
| S2.1 — Value objects/domain model | **Partial.** [Parsed IR](../packages/compiler/src/infrastructure/postcss-stylesheet-parser.ts#L14), [logical Module identity](../packages/compiler/src/application/module-identity.ts), [naming identities](../packages/compiler/src/domain/readable-name.ts) and [public schema/diagnostics](../packages/compiler/src/public-types.ts) exist; [Module identity tests](../packages/compiler/test/module-identity.test.ts) cover relocation. | Establish framework-independent domain/port IR invariants for all named concepts, source ranges and byte-stable normalized-IR fixtures. Parsed IR is currently infrastructure-owned, with paths/layers/specificity distributed as arrays/strings/numbers; classes or branding alone are not the gate. |
| S2.2 — CSS parsing behind port | **Partial.** PostCSS/nesting [parser](../packages/compiler/src/infrastructure/postcss-stylesheet-parser.ts#L87) is called directly by the session; [session tests](../packages/compiler/test/compiler-session.test.ts#L1697) cover nesting and lists. | Implement/test the accepted `CssParserPort` boundary and parser error contract; the parser function is not an injected port. |
...
| S2.6 — Full accepted selector parsing | **Partial.** [Selector parser](../packages/compiler/src/infrastructure/postcss-stylesheet-parser.ts#L304) and [session assertions](../packages/compiler/test/compiler-session.test.ts#L1168) cover local paths, equality attributes, states, simple functional branches, pseudos and simple observations. | General residual and explicit-global syntax, interleaving and accepted mixed compositions. Functional arguments currently have one pseudo/attribute per branch; observed branches have one node, optionally one local state. Implement normalization and safety validation together, not acceptance by emission alone. |
| S2.7 — Stable diagnostics | **Partial.** [Diagnostic type](../packages/compiler/src/public-types.ts#L1) and [malformed/duplicate rollback tests](../packages/compiler/test/compiler-session.test.ts#L96) establish bounded codes. | Source ranges/path attribution, deterministic range/code/path sorting and path-clean messages, matrix-wide diagnostics/rollback tests. Physical ids/upstream parser messages still escape the target path-clean contract. Verify malformed selector-AST exceptions cannot escape: the parser catch covers PostCSS, not the later `astSync` call. |

The checked list/nesting labels remain bounded normalization work, not proof of every accepted nesting/composition form. Include a mixed valid/invalid selector-list fixture in the whole-Module fail-closed gate.
...
| S4.9 — Interleaved chains/global branches | **Not implemented.** [Session validation](../packages/compiler/src/application/compiler-session.ts#L355) rejects ownership after runtime edges; [functional/observed parser](../packages/compiler/src/infrastructure/postcss-stylesheet-parser.ts#L383) has no global branch. [Prefix test](../packages/compiler/test/compiler-session.test.ts#L1554) proves only a leading ownership prefix. | Implement accepted interleaved marker/scoping and explicit-global functional/observed branches with positive and ambiguity/rollback fixtures. |
| S4.10 — Residual/global planning | **Partial.** Bounded residual observation exists in [has parser](../packages/compiler/src/infrastructure/postcss-stylesheet-parser.ts#L426) and [session tests](../packages/compiler/test/compiler-session.test.ts#L872). | General residual tag/attribute and explicit-global contextual placement/scoping with positive and negative tests; do not treat the bounded observed residual as the entire item. |
...
| Original item | Status and existing evidence | Remaining gate for that same item |
| --- | --- | --- |
| S5.1 — Session factory | **Completed; newly checked.** [Factory](../packages/compiler/src/application/compiler-session.ts#L100) owns the contribution map/generation; [public-session test](../packages/compiler/test/compiler-session.test.ts#L20) exercises it. | None for the factory label. Constructor port injection remains S2/S5 architecture work. |
