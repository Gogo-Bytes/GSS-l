...
| Original item | Status and existing evidence | Remaining gate for that same item |
| --- | --- | --- |
| S3.4 — Full property-effect classification | **Partial.** [Versioned classifier](../packages/compiler/src/domain/property-effects.ts#L164), [effect tests](../packages/compiler/test/property-effects.test.ts) and [strict/preserved session tests](../packages/compiler/test/compiler-session.test.ts#L416) prove unknowns cannot enter session atomic output. A bounded v1 `font`/`font-variant` reset audit now covers all six represented variant effects and public session/native-golden forward, reverse and important cases. A table-driven completeness invariant now enumerates the currently registered shorthand/effect families, checks duplicate-free represented effects and known longhand classification for every listed effect, and preserves unknown-property fallback behavior; it does not complete the registry audit. | Audit the full positive dataset and remaining shorthand overlap/reset effects with table coverage. Family presence/sample winners are not completeness proof. Lower-level `effectsOfProperty()` still has a singleton fallback; do not confuse it with `classifyPropertyEffect()`'s explicit unknown result. |
| S3.6 — Importance/layers | **Partial.** [Closed-target resolver](../packages/compiler/src/domain/resolve-target-declarations.ts), [descendant priority tests](../packages/compiler/test/descendant-conditions.test.ts#L183), [layer output tests](../packages/compiler/test/compiler-session.test.ts#L628), [simple-layer fixtures](../packages/testing/browser/condition-layer-fixtures.ts), and new [nested canonical-layer goldens](../packages/testing/browser/nested-layer-fixtures.ts) cover bounded normal/important/unlayered behavior, including configured dotted names. | Cross-context candidates composed with the complete specificity/condition/relation model and broader layer corpus. Preserve native important-layer reversal; do not rebuild the already present layer emitter. |
| S3.7 — Specificity including where | **Partial.** [Descendant planner](../packages/compiler/src/domain/plan-descendant-conditions.ts), [where test](../packages/compiler/test/compiler-session.test.ts#L1281), and [observed-list specificity regression](../packages/compiler/test/has-specificity.test.ts) cover bounded ownership/predicate specificity and target-qualified emission of the accepted `:has()` selector-list maximum without strengthening reusable atoms. Repeated observed identities retain distinct emitted selectors when their list maxima differ; exact-selector deduplication remains tested. | General accepted-selector specificity, including residual/global and functional/observed-list composition, and cross-layer/condition candidate acceptance. Parent native acceptance now includes the hand-authored observed-list fixtures and their specificity controls in the **94 fixtures / 1027 comparisons** corpus; this bounded gate does not prove the full cross-context priority matrix. |
| S3.10 — Full implication/ambiguity | **Partial.** Existing [predicate planner](../packages/compiler/src/domain/plan-descendant-conditions.ts) plus the bounded [structural proof planner](../packages/compiler/src/domain/plan-structural-relations.ts) now prove aligned adjacent ⇒ general sibling, additional source predicates and runtime-suffix refinement. [Public session tests](../packages/compiler/test/structural-relations.test.ts) cover coactivity, effect overlap, source witnesses, closed equivalence, priority and LKG. | General structural/functional/observed implication, remaining mixed-predicate intersections/exclusions and matrix-wide transactional coverage remain open. Opaque observed branches participate in affected structural conflict validation, not a general observed solver. Names never resolve the bounded ambiguous pairs. |

### Bounded structural relation follow-up

A public-session red tracer reproduced `.input + .label { color: red } .input ~ .label { color: blue }` emitting red before blue, contrary to ADR-0013. Parent native red independently confirmed adjacent blue instead of literal red in six new forward/reverse base/state/attribute goldens; the original 49 fixtures remained clean. The repair separates ownership prefixes from runtime witness edges, projects proved implication to emission rank, retains authored specificity/provenance, and rejects incomparable coactive equal-priority effect conflicts before commit or preserved fallback. General-sibling sources may be different DOM nodes: exclusions/intersections require a proved identical unique reverse witness, not equal scope/class names. Same-target structural/observed competition is validated with emitted branch class/type specificity without adding observed implication.

`corepack pnpm verify` passes **822 tests** (353 Compiler, 326 reference, 23 React, 120 Vite), lint/build/typecheck. Before review corrections, parent opened the rebuilt owned strict4178 harness with native `agent_browser`: **64 fixtures / 953 comparisons**, zero differences/literal failures, all **seven** controls effective. Original **49 / 875 / six controls** remain intact; twenty new [hand-authored contextual goldens](../packages/testing/browser/structural-relation-fixtures.ts) independently encode ADR-0013 winners, not native authored order or an expanded reference API. Review corrections retain context-independent shared descendant ranks and permit equal-native-priority full intersections that strictly refine both conditions on one unique witness. Seven added public regressions and five added goldens cover shared margin effects (registration/replacement/single-Module) and zero-specificity intersections, retaining repeated-witness/extra-predicate negatives. After the repair/build, parent native `agent_browser` freshly passed **69 fixtures / 972 comparisons**, zero differences/literal failures, all seven controls detected. The three shared-effect records read margin-left 2px → 1px → 2px, and both equal-specificity intersections pass all phases. Targeted wrong-order corruption changes only adjacent red to blue, with distant readings unchanged. Full S3.10 remains partial; **108 original checkbox units / 87 checked / 21 open** are unchanged.
...
| S4.9 — Interleaved chains/global branches | **Partial.** [Session tests](../packages/compiler/test/compiler-session.test.ts#L1735) cover one root-level `+` runtime edge followed by two ownership-descendant edges in `.input + .label .icon .badge`, plus the same bounded suffix after a leading ownership prefix in `.root .input + .label .icon`; full paths and intermediate/final markers are retained, declaration reversal is deterministic, and another runtime edge or relation kind still fails closed. Parent native acceptance passed **96 fixtures / 1031 comparisons**, zero differences/literal failures, and all seventeen controls for the root-source and leading-prefix slices, including adjacent, general-sibling and child shapes. The parser still has no explicit-global functional/observed branch. | Other interleaved runtime/prefix shapes and explicit-global functional/observed branches with positive and ambiguity/rollback fixtures. |
...
Nested canonical-layer acceptance is now covered by four additional hand-authored browser fixtures across source-block and configured-order reversals; native verification and its isolated prelude-order control are recorded in the [testing README](../packages/testing/README.md#nested-canonical-layer-goldens). This is Compiler-host evidence, not an expansion of the independent reference's flat simple-layer syntax.
