...
| S4.9 — Interleaved chains/global branches | **Partial.** [Session tests](../packages/compiler/test/compiler-session.test.ts#L1697) and two [hand-authored native goldens](../packages/testing/browser/interleaved-chain-fixtures.ts) cover one `+` runtime edge followed by one ownership-descendant edge in `.input + .label .icon`, preserving the full `input.label.icon` path and source/context/target markers. Parent native acceptance passed **84 fixtures / 1007 comparisons**, zero differences/literal failures, and all eleven controls; an adjacent-edge removal control changes only the target color. [Session validation](../packages/compiler/src/application/compiler-session.ts#L355) still rejects broader post-runtime chains; the parser has no explicit-global functional/observed branch. | Broader interleaved marker/scoping and explicit-global functional/observed branches with positive and ambiguity/rollback fixtures. |
