...
The bounded reference slices and forty-three-fixture native browser acceptance are not the complete oracle or Pilot gate. The targeted production descendant-condition repair is implemented; contextual source/observed subject emission now preserves the same ownership-prefix specificity without changing marker placement/matching, at-rule semantics or resources. `packages/testing/README.md` documents its exact input limits, reproducible startup and machine-readable browser result. `resolveAssetUrl` is an approved port shape but resource support is not implemented here; nonempty asset bindings and condition/layer configurations outside the exact ADR-0030/testing README slice fail explicitly. Cross-kind/compound/nested reference ordering, the full React corpus, browser CI and Pilot remain incomplete; no new production capability was deferred.
