...
- 有界链 `.input + .label .icon` 按 ADR-0010 保留完整 `input.label.icon` ownership path：source marker 落在 `input`，中间 contextual marker 与 target marker 分别落在 `label` 和 `icon`，输出保留 `+` 后的普通 descendant 空格。只有 `input + label` 是 runtime relation；末尾 `label` 到 `icon` 的 descendant 是 ownership suffix，不参与 runtime implication。当前仅接受一个 adjacent edge 后接一个 ownership-descendant edge，不推及任意更长或更多 runtime edge 的链。
