...
- 有界链 `.input + .label .icon` 按 ADR-0010 保留完整 ownership path：source marker 落在 `input`，后续 ownership 段分别保留 contextual/target marker；输出保留 `+` 后普通 descendant 空格。只有 `input + label` 是 runtime relation；其后的一个或多个 descendant edge 是 ownership suffix，不参与 runtime implication。当前 Compiler 接受根 source 后恰好一个 adjacent edge，后接一个或多个 ownership-descendant edges；不由此推及 leading ownership prefix、additional runtime edges 或其他 interleavings。
