...
- 有界链 `.root .input + .label .icon`、`.root .input ~ .label .icon` 与 `.input > .label .icon` 按 ADR-0010 保留完整 ownership path：source marker 落在 runtime source，leading prefix 时按完整 source path 重复以保留 ownership specificity；后续 ownership 段分别保留 contextual/target marker，输出保留 runtime combinator 后普通 descendant 空格。只有一个 adjacent、general-sibling 或 root-level child edge 是 runtime relation；其后的一个或多个 descendant edge 是 ownership suffix，不参与 runtime implication。当前 Compiler 接受全 descendant leading ownership prefix 加 adjacent/general-sibling edge，或无 prefix 的 root-level child edge；不由此推及 prefix+child、additional runtime edges 或其他 interleavings。
