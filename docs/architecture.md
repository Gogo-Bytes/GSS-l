...
Module invalidation removes its references; zero-reference atoms, markers, and resources disappear from the next snapshot. Per-physical-file generation tokens prevent stale async reads from committing after a newer replacement or deletion. Physical change events also invalidate virtual JS and recorded source importers, ensuring fresh ScopeSchema validation. After a latest successful replacement with unchanged committed ScopeSchema, only native CSS publication propagates; identical-byte error recovery forces that publication to clear the native overlay. Changed ScopeSchema and deletion still propagate to JavaScript consumers. This bounded repair follows a reproduced baseline defect: unconditional propagation reloaded a non-accepting importer even on identical-source recovery. Virtual loads respect Vite's filesystem access policy.
...
Public ranges are zero-based UTF-16, half-open in original caller CSS (including BOM/CRLF/astral units). `GSS1001` CSS syntax translates only validated `error.input` line/column coordinates whose input text matches caller CSS after one BOM removal, never remapped upstream fields. Supplied ends must be valid/exclusive; missing ends are zero-width parser-reported points, including EOF if actually reported. Invalid/partial/reversed/out-of-bounds/split-surrogate coordinates omit the range. Selector syntax reached in prevalidation and `GSS1101` unsupported selectors reuse the complete enclosing authored rule span; nested/list branches do not promise exact selector-token locations. Map-helper errors without Input provenance remain unranged even inside selector prevalidation. `GSS1204` uses the actual repeated declaration span. Other capability/config/binding/resource/registry/multi-origin sites remain unranged. Existing emission order and transaction semantics are unchanged. See [public tracers](../packages/compiler/test/diagnostic-ranges.test.ts). This does not implement CSS maps, source-to-output tracing, IDE integration or matrix-wide S2 attribution. Native Vite presentation is an Adapter-owned follow-up, described below.

The [Vite diagnostic presenter](../packages/vite/src/diagnostics.ts) consumes those optional ranges through native Rollup/Vite `loc` and `frame` fields. Replacement results carry their exact owned CSS text; final production census uses retained Module metadata, never a later file read for formatting. The snapshot id must equal the diagnostic owner. LF-delimited lines become one-based lines/zero-based UTF-16 columns; CRLF, BOM and astral offsets stay in original caller coordinates. Frames preserve tabs, bound long lines/context, respect multiline exclusive ends and show one insertion caret for zero-width points (including terminal-newline EOF), without widening the public range. Missing/mismatched snapshots and invalid/reversed/out-of-bounds/split-surrogate ranges remain location-free. Explicit native plugin identity/code and a prebuilt frame prevent an importing JSX source map or host source reread from replacing the CSS attribution. React diagnostics and IO/resource errors do not borrow a CSS snapshot. The Compiler has no Vite formatting dependency or new source-adapter fields. [Real host regressions](../packages/vite/test/diagnostic-location.test.ts) exercise load, JSX with an upstream map, build/watch, native watcher errors, source replacement races and census warning snapshots; [private defensive tests](../packages/vite/test/diagnostics.test.ts) cover synthetic unreliable/EOF inputs, not browser acceptance.
