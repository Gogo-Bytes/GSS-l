...
**Nested named-layer Compiler acceptance passed:** four hand-authored native CSS fixtures exercise configured `framework.base`/`framework.utilities` names across source-block and configuration order reversals. Parent native `agent_browser` passed the complete **82 fixtures / 1003 comparisons** corpus, zero differences/literal failures, with all ten controls detected. The new nested-layer control perturbs only the dotted layer prelude: the three layered normal/important winners change as expected, while the unlayered-normal winner remains unchanged. Literal readings cover normal configured precedence, important layer reversal, unlayered normal dominance, and layered-important dominance over unlayered important. Existing 78/987 corpus and nine controls remain intact. The Compiler already implemented the ADR-0026 contract; this was an acceptance-only slice, not a production behavior fix or expansion of the independent reference, which still covers only flat/simple layer fixtures. Full S3.6/S4.16 and browser CI/Pilot remain open.
...
**Bounded interleaved-chain Compiler slice (ADR-0010/0012):** the first public-session increment accepted `.input + .label .icon`, retaining the full path and excluding the trailing ownership descendant from runtime-implication edges. Reversing authored rule order retained identical CSS/schema; two hand-authored native goldens and a CSSOM adjacent-edge control passed parent native acceptance at **84 fixtures / 1007 comparisons**, zero differences/literal failures, and all eleven controls. That gate covers only the one-descendant suffix.

The next bounded increment accepts `.input + .label .icon .badge`: exactly one root-level adjacent runtime edge, followed by two ownership-descendant edges. It retains the full target path plus `label`/`icon` contextual markers and final `badge` marker; reversing declaration-rule order retains identical CSS/schema. A further runtime edge after or splitting the ownership suffix still fails closed. The public-session test verifies exact CSS/schema and both rejection cases. Two hand-authored deep browser fixtures independently specify red/blue computed values in forward/reversed declaration order. Parent native `agent_browser` passed **86 fixtures / 1011 comparisons**, zero differences/literal failures, and all **twelve** controls; the prior 84/1007 corpus and eleven controls remain intact. The dedicated CSSOM control collapses only the last descendant edge, changing only badge color from red to black. S4.9 remains partial: other interleaved runtime/prefix shapes and explicit-global branches remain open.

**Leading-prefix interleaved Compiler slice (ADR-0010/0012):** the next bounded public-session case accepts `.root .input + .label .icon`, with a full descendant ownership prefix before the one adjacent runtime edge and one descendant ownership suffix afterward. The source marker is attached to the runtime source path `root.input` and repeated by prefix depth; contextual/target markers retain the normalized runtime relation identity (`adjacent/descendant`) and full target path. Declaration reversal is deterministic, while a second runtime edge and `>` variants remain fail-closed. Two hand-authored native fixtures and an isolated adjacent-edge control are added. Parent native `agent_browser` passed **88 fixtures / 1015 comparisons**, zero differences/literal failures, and all **thirteen** controls; the earlier 86/1011 corpus and twelve controls remain intact. The control removes only the adjacent edge and changes only icon color from red to black. No explicit-global branch or broader interleaving is claimed.

**General-sibling leading-prefix interleaved slice:** the bounded Compiler now also accepts `.root .input ~ .label .icon`, retaining the repeated `root.input` source marker, normalized `general-sibling/descendant` contextual identity, and full target path. Public-session tests cover exact CSS/schema, reversed declaration order, while root-level general-sibling suffixes, `>` variants, and extra runtime edges remain rejected. Two hand-authored native fixtures and a CSSOM control removing only `~` pass parent native `agent_browser` at **90 fixtures / 1019 comparisons**, zero differences/literal failures, and all **fourteen** controls; the control changes only icon color from red to black. Explicit-global branches remain open.

**Root general-sibling suffix Compiler slice:** the bounded session now also accepts `.input ~ .label .icon`, with exactly one root-level general-sibling runtime edge followed by an ownership descendant suffix. Public tests verify exact CSS/schema and declaration reversal; a second runtime edge, child relation, and other unsupported root suffixes remain fail-closed. Two hand-authored native fixtures and an isolated `~` removal control passed parent native `agent_browser` acceptance: **92 fixtures / 1023 comparisons**, zero differences/literal failures, and all **fifteen** controls; the earlier 90/1019 corpus and fourteen controls remain intact. The control removes only `~` and changes only icon color from red to black. Explicit-global branches remain open.

**Root child suffix Compiler slice:** the public session now accepts `.input > .label .icon`, with exactly one root-level child runtime edge followed by an ownership descendant suffix. Exact CSS/schema, declaration reversal and fail-closed second-runtime/split-suffix/prefix-child cases are public-tested. Two hand-authored native fixtures and an isolated child-edge corruption control passed parent native `agent_browser` acceptance: **94 fixtures / 1027 comparisons**, zero differences/literal failures, and all **sixteen** controls; the earlier 92/1023 corpus and fifteen controls remain intact. The control changes only the child edge to an adjacent edge and changes only icon color from red to black. Prefix-child and explicit-global branches remain open.

**Leading-prefix child suffix Compiler slice:** the public session now also accepts `.root .input > .label .icon`, retaining the full `root.input.label.icon` path, repeated source marker and normalized `child/descendant` contextual identity. Reversed declaration order is deterministic; extra runtime edges and split suffixes remain fail-closed. Two hand-authored native fixtures use a non-void `div` source with a direct child label, plus a CSSOM control that changes only the child edge to an adjacent edge. Parent native `agent_browser` passed **96 fixtures / 1031 comparisons**, zero differences/literal failures, and all **seventeen** controls; the earlier 94/1027 corpus and sixteen controls remain intact. The control changes only icon color from red to black. Prefix-child does not expand the accepted global branch.
