...
**Bounded interleaved-chain Compiler slice (ADR-0010/0012):** the public session accepts exactly `.input + .label .icon`: the full `input.label.icon` path is retained; a source marker plus intermediate `label` contextual marker and final `icon` target marker produce the authored adjacent edge followed by an ordinary ownership descendant. The descendant suffix is excluded from structural runtime-implication edges. Reversing authored declaration-rule order retains identical CSS and scope identity. Two-or-more trailing ownership-descendant edges remain fail-closed. `compiler-session.test.ts` verifies exact CSS/schema and rejection diagnostics. Two hand-authored native browser fixtures independently verify exact red/blue computed values in forward/reversed authored rule order. Parent native `agent_browser` passed **84 fixtures / 1007 comparisons**, zero differences/literal failures, all **eleven** controls detected; the new CSSOM control removes only the adjacent edge and changes only the target color from red to the baseline black. The original 82/1003 corpus and ten controls remain intact. This is bounded S4.9 acceptance only; explicit-global branches and broader interleaving remain unimplemented.
