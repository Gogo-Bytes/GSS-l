...
- 当前 reference 对上述范围外的 syntax、value functions/escapes、下述有界 condition/layer slice 以外的 config 与 asset bindings 显式报错；resolver port 暂不调用。这是 reference coverage 边界，不改变上文 GSS product capabilities；reference 不是完整 GSS validator，也不解析 state ambiguity。
