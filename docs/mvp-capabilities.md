...
本文件列出第一版已接受的正向能力契约，**不是所有能力已实现或已通过完整验收的清单**。未列入的语法按 fail-closed 处理；已明确讨论并延后的能力见 [deferred-capabilities.md](deferred-capabilities.md)。实现与验收状态以 [MVP roadmap](mvp-roadmap.md#reading-status-and-counts) 为准；以下状态核对基于当前已推送的 bounded slices，不改变 accepted ADR 或新增延期项。
...
- **Implemented bounded structural priority:** aligned `+` implies `~` on the same owned target; source predicate supersets and aligned runtime-suffix refinement generate proof-derived emission order, not combinator weights. Unique reverse witnesses permit bounded exclusions/intersections; multiple same-scope source nodes remain potentially coactive. Same-context structural/descendant and structural/observed equal-priority effect conflicts fail transactionally; native importance/specificity, wrappers and ownership/subject isolation remain intact. [Public regressions](../packages/compiler/test/structural-relations.test.ts) and twenty labelled hand-authored browser goldens supplement the unchanged independent reference API. Historical structural-priority verification: **822 tests**; the corresponding post-review parent native gate was **69 fixtures / 972 comparisons / seven effective controls**, retaining the original **49 / 875 / six controls**. The current bounded interleaved gate is recorded below. Review corrections preserve shared descendant ranks and accept equal-native-priority, strictly refining full intersections on a unique witness; five new goldens pass the fresh parent native gate. This implements a bounded ADR-0013 slice, not full structural/functional/observed implication or S3 completion.
- **Current bounded interleaved acceptance:** one runtime edge (`>`, `+`, or `~`) followed by one-or-more ownership-descendant edges is accepted for the tested root and leading-prefix shapes; full paths, marker identities, order reversal and native computed winners are covered by **842 tests** and the parent **96 fixtures / 1031 comparisons / 17 controls** gate. Prefix/runtime combinations outside this bound and explicit-global branches remain fail-closed and open.
- **Not implemented:** explicit-global functional/observed branches and broader interleaved ownership/runtime combinations beyond the bounded slices above; browserslist compatibility transformation/indivisible sequences; compatibility-specific whole-Module fallback. Property-effect fallback is implemented separately.
- **Partial architecture/output:** the first internal `CssParserPort` is injected into the session with default PostCSS composition outside the use case; domain-owned normalized IR now carries original UTF-16 half-open declaration/rule/resource/frame spans ([granularity](architecture.md#internal-source-provenance-bounded-implementation)). The public constructor remains unchanged. Optional public `GssSourceRange` is now supported for bounded `GSS1001` syntax, `GSS1101` unsupported selectors and `GSS1204` duplicate declarations ([ADR-0051](adr/0051-expose-bounded-original-css-diagnostic-ranges.md)): original caller UTF-16 half-open offsets, actual repeated declaration or enclosing authored rule granularity, and zero-width parser-reported points when no end exists. Unreliable/map-helper origins and other sites omit ranges; id/code/order/LKG remain unchanged. The full value model, public port suite, `NameAllocatorPort` wiring and complete reversibility proof remain open. Compiler CSS source maps, remaining diagnostic-site attribution, IDE integration and richer target manifest/report fields are absent; current version-1 output is not that full target.
- **Implemented bounded Vite diagnostic presentation:** optional Compiler ranges become native physical-file `loc`/`frame` using the exact replacement or census source snapshot, including importing JSX with its own source map. No extra source reads, public fields or custom overlay protocol. Invalid/unavailable/unmatched ranges and React/IO diagnostics remain location-free. Native error recovery with unchanged ScopeSchema is CSS-only (including identical bytes); changed-schema/deletion propagation remains intact. [Host regressions](../packages/vite/test/diagnostic-location.test.ts) and [defensive coordinate tests](../packages/vite/test/diagnostics.test.ts) supplement—not replace—full host acceptance.
