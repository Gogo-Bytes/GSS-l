...
A CSSOM negative control removes only the emitted adjacent `+` edge. It must leave the reference unchanged and change only the atomic target's color from red to the `div` baseline black; the background remains blue. Parent native `agent_browser` passed **84 fixtures / 1007 comparisons**, zero differences/literal failures, and all **eleven** controls detected. The prior 82/1003 corpus and ten controls remain intact. This is only the one-adjacent-edge/one-owned-descendant S4.9 slice; broader interleaved chains and explicit-global branches remain open.
