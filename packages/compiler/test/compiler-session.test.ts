...
describe('GssCompilerSession', () => {
  ...
  it('retains a leading ownership prefix around an adjacent runtime relation and suffix', () => {
    const sources = [
      ...
      '.root .input + .label .icon { background-color: blue; } .root .input + .label .icon { color: red; }'
    ];
    const results = sources.map((source) => {
      const compiler = createGssCompilerSession({ projectRoot: '/project' });
      const replacement = compiler.replaceStylesheet({
        id: '/project/src/prefix.gss',
        source
      });
      return { replacement, css: compiler.finalize().css };
    });

    const sourceMarker = 'gss-s--module_src_2f_prefix_2e_gss--path_root_2f_input';
    const contextMarker =
      'gss-c--module_src_2f_prefix_2e_gss--relation_adjacent_2f_descendant--position_1--path_root_2f_input_2f_label--target_root_2f_input_2f_label_2f_icon';
    const targetMarker =
      'gss-t--module_src_2f_prefix_2e_gss--relation_adjacent_2f_descendant--source_root_2f_input--target_root_2f_input_2f_label_2f_icon';
    const selector = `.${sourceMarker}.${sourceMarker} + .${contextMarker} .${targetMarker}`;
  ...
  it('retains a leading ownership prefix around a general-sibling runtime relation and suffix', () => {
    const sources = [
      '.root .input ~ .label .icon { color: red; } .root .input ~ .label .icon { background-color: blue; }',
      '.root .input ~ .label .icon { background-color: blue; } .root .input ~ .label .icon { color: red; }'
    ];
    const results = sources.map((source) => {
      const compiler = createGssCompilerSession({ projectRoot: '/project' });
      const replacement = compiler.replaceStylesheet({ id: '/project/src/prefix-gs.gss', source });
      return { replacement, css: compiler.finalize().css };
    });
    const sourceMarker = 'gss-s--module_src_2f_prefix_2d_gs_2e_gss--path_root_2f_input';
    const contextMarker =
      'gss-c--module_src_2f_prefix_2d_gs_2e_gss--relation_general_2d_sibling_2f_descendant--position_1--path_root_2f_input_2f_label--target_root_2f_input_2f_label_2f_icon';
    const targetMarker =
      'gss-t--module_src_2f_prefix_2d_gs_2e_gss--relation_general_2d_sibling_2f_descendant--source_root_2f_input--target_root_2f_input_2f_label_2f_icon';
    const selector = `.${sourceMarker}.${sourceMarker} ~ .${contextMarker} .${targetMarker}`;
    const expectedSchema = {
      root: { selfClassName: '', targets: {
        input: { selfClassName: sourceMarker, targets: {
          label: { selfClassName: contextMarker, targets: { icon: { selfClassName: targetMarker, targets: {} } } }
        } }
      } }
    };
  ...
  it('retains a leading ownership prefix around a child runtime relation and suffix', () => {
    const sources = [
      '.root .input > .label .icon { color: red; } .root .input > .label .icon { background-color: blue; }',
      '.root .input > .label .icon { background-color: blue; } .root .input > .label .icon { color: red; }'
    ];
    const results = sources.map((source) => {
      const compiler = createGssCompilerSession({ projectRoot: '/project' });
      const replacement = compiler.replaceStylesheet({ id: '/project/src/child-prefix.gss', source });
      return { replacement, css: compiler.finalize().css };
    });
    const sourceMarker = 'gss-s--module_src_2f_child_2d_prefix_2e_gss--path_root_2f_input';
    const contextMarker =
      'gss-c--module_src_2f_child_2d_prefix_2e_gss--relation_child_2f_descendant--position_1--path_root_2f_input_2f_label--target_root_2f_input_2f_label_2f_icon';
    const targetMarker =
      'gss-t--module_src_2f_child_2d_prefix_2e_gss--relation_child_2f_descendant--source_root_2f_input--target_root_2f_input_2f_label_2f_icon';
    const selector = `.${sourceMarker}.${sourceMarker} > .${contextMarker} .${targetMarker}`;
    const expectedSchema = {
      root: {
        selfClassName: '',
